...
if SRM_detected:
    ...
    with st.expander("ℹ️ What is a Sample Ratio Mismatch?"):
        ...
        **Best Practice:** Pause the test, investigate the allocation logic, and re-run it once fixed. This test's results should be considered inconclusive. SRM breaks the assumptions of random assignment and renders the outcome invalid.
        """)

# Prior is now derived above from optional past data
samples = 10000

# --- Priors derived automatically from Variant A observed data ---
alpha_prior = 1 + conv_A
beta_prior = 1 + n_A - conv_A
post_A = beta(alpha_prior + conv_A, beta_prior + n_A - conv_A)
post_B = beta(alpha_prior + conv_B, beta_prior + n_B - conv_B)  # Uses same prior as A
